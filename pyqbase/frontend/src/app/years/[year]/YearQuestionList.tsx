...
interface YearQuestionListProps {
  year: number
}

export function YearQuestionList({ year }: YearQuestionListProps) {
  const searchParams = useSearchParams()
  const initialExam = searchParams.get('exam') || null

  const [selectedExam, setSelectedExam] = useState<string | null>(initialExam)
  ...
  const { data, isFetching, isError } = useQuery<any>({
    queryKey: ['year-questions', year, selectedExam, offset],
    queryFn: async () => {
  ...
  const EXAMS = [
    ...
    { id: "MPSC Rajyseva", name: "MPSC Rajyseva" },
  ...
  return (
    <div className="space-y-8 mt-12">
      {/* Exam Filter */}
      <div>
        <h3 className="text-xl font-bold tracking-tight mb-4">Filter by Exam</h3>
      ...
      <div className="flex items-center justify-between border-t pt-10">
        <div>
          <h2 className="text-3xl font-bold tracking-tight mb-2">Questions from {year}</h2>
          <p className="text-muted-foreground">Browse all historical questions asked in {year}.</p>
        </div>
      </div>
